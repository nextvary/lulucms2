...
class Menu extends \source\core\base\BaseActiveRecord
{
    ...
    public static function getParentMenuByUrl($url){
        $url=explode('/',$url)[0];
        $menu=Menu::find()->where(['like','url',"/".$url.'%',false])->one();
        start:
        if($menu && $menu->parent_id!=0){
            $menu=self::find()->where(['id'=>$menu->parent_id])->one();
            goto start;
        }
        return $menu;
    }
    ...
    public static function getArrayTreeInternalWithUrl($category,$url,$fromCache = true)
    {
...
//     }
    ...
    public static function getChildrenWithRole($role,$category,$parentId,$status=null)
    {
        ...
        foreach ($menus as $menu) {
            foreach ($actions as $action) {
                ...
                $true1=strpos($menu['url'],'/'.$action['permission']);
    ...
    public static function getAdminMenuByRole($role)
    {
        $html='';
        ...
        $roots= (new RbacService())->getParentMenuByRole($role);
        ...
        foreach ($roots as $menu)
        {
            ...
            $children = self::getChildrenWithRole($role,'admin',$menu['id'],1);
            if(count($children)>0)
            {
                $opened=false;
                $childHtml='';
